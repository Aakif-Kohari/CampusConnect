...
import { verifySignature, parseAuthenticatorData, verifyRpIdHash } from "../shared/crypto-verify.ts";
...
serve(async (req: Request) => {
    ...
    try {
        ...
        const { data: challengeRecord, error: challengeFetchError } =
            await supabase
        ...
        if (
            challengeRecord.user_id !== null &&
            challengeRecord.user_id !== credential.user_id
        ) {
        ...
        if (!parsedAuthData.userPresent) {
            return new Response(
                JSON.stringify({ error: "User was not present during authentication" }),
                {
                    status: 400,
                    headers: { ...corsHeaders, "Content-Type": "application/json" },
                },
        ...
        if (
            ...
            parsedAuthData.signCount <= storedCounter
        ) {
            return new Response(
                JSON.stringify({
                    error:
                        "Possible cloned authenticator detected (counter did not increment)",
                }),
                {
                    status: 400,
                    headers: { ...corsHeaders, "Content-Type": "application/json" },
        ...
        // 8. Delete the consumed challenge (one-time use)
        await supabase
            .from("webauthn_challenges")
            .delete()
