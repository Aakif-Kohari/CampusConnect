...
serve(async (req: Request) => {
    ...
    try {
        ...
        if (!credentialId || !clientDataJSON || !authenticatorData) {
            console.error({ step: "validate_required", reason: "Missing required credential data", received: { credentialId: !!credentialId, clientDataJSON: !!clientDataJSON, authenticatorData: !!authenticatorData } });
            return new Response(
                JSON.stringify({ error: "Missing required credential data" }),
                {
                    status: 400,
                    headers: { ...corsHeaders, "Content-Type": "application/json" },
                },
        ...
        if (!origin) {
            console.error({ step: "validate_origin_presence", reason: "Missing origin parameter in body" });
        ...
        if (challengeFetchError) {
            ...
            return new Response(
                JSON.stringify({ error: "Challenge validation failed" }),
                {
                    status: 500,
                    headers: { ...corsHeaders, "Content-Type": "application/json" },
                },
        ...
        if (!challengeRecord) {
            console.error({ step: "challenge_lookup", reason: "No pending registration challenge found, or it has expired", user_id: user.id });
            return new Response(
        ...
        try {
            ...
            return new Response(
                JSON.stringify({ error: "Invalid clientDataJSON" }),
                {
                    status: 400,
                    headers: { ...corsHeaders, "Content-Type": "application/json" },
                },
        ...
        if (clientData.type !== "webauthn.create") {
            console.error({ step: "clientData_type", reason: "Invalid clientData type", expected: "webauthn.create", received: clientData.type });
            return new Response(
        ...
        if (clientData.challenge !== challengeRecord.challenge) {
            console.error({ step: "challenge_match", reason: "Challenge mismatch", clientChallenge: clientData.challenge, recordChallenge: challengeRecord.challenge });
            return new Response(
        ...
        if (clientData.origin !== origin) {
            console.error({ step: "origin_match", reason: "Origin mismatch", clientOrigin: clientData.origin, expectedOrigin: origin });
            return new Response(
        ...
        try {
            ...
            if (!parsedAuthData.userPresent) {
                console.error({ step: "authenticator_data", reason: "User was not present during registration" });
                throw new Error("User was not present during registration");
            }
        } catch (e) {
            console.error({ step: "authenticator_data", reason: `Invalid authenticator data: ${(e as Error).message}` });
            return new Response(
        ...
        if (existingCred) {
            console.error({ step: "duplicate_credential", reason: "This passkey is already registered", credentialId });
            return new Response(
        ...
        if (!publicKeyB64url) {
            console.error({ step: "extract_public_key", reason: "Failed to extract public key (parsedAuthData.publicKeyBytes is null or empty)" });
            return new Response(
                JSON.stringify({ error: "Failed to extract public key" }),
                {
                    status: 400,
                    headers: { ...corsHeaders, "Content-Type": "application/json" },
                },
        ...
        if (insertError) {
            console.error({ step: "db_insert", reason: "Failed to store credential", error: insertError });
            return new Response(
