...
self.addEventListener("push", (event) => {
  ...
  try {
    ...
    // Support non-JSON fallback for announcements
    data = { title: "CampusConnect Announcement", body: event.data?.text() || "You have a new notification.", data: { url: "/" } };
  }

  const title = data.title || "New Notification";
  // Determine if it's an announcement (has message) or DM (has body)
  const options = {
    body: data.body || data.message || "Click to view your messages.",
    icon: data.icon || "/icon-192x192.png",
    badge: data.badge || "/icon-192x192.png",
    // Merge data and url properties from both payload styles
